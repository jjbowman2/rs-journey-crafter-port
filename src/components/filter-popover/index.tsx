  ...
  const remainingAttributes = useMemo(
    () =>
      FilterAttribute.options.filter(
        (attribute) => !filters.some((filter) => filter.attribute == attribute)
      ),
    [filters]
  );
    ...
    <Popover placement="bottom-start">
      <PopoverTrigger>
        <div>
          ...
          />
          {!!filters.length && (
            <Badge marginLeft={-4} marginTop={-4} colorScheme="orange">
              {filters.length}
            </Badge>
          )}
        </div>
        ...
        <PopoverBody>
          <Stack>
            ...
            <FormControl isInvalid={Boolean(formError)}>
              ...
              <InputGroup>
                <Select
                  ...
                  flex={2}
                  onChange={(e) =>
                    setFilterAttribute(e.target.value as FilterAttribute)
                  }
                >
                  {remainingAttributes.length > 0 ? (
                    ...
                    <option disabled>No filters remaining</option>
                  )}
                </Select>
                <Select
                  flex={1}
            ...
            {filters.map((filter, index) => (
              <Flex key={`${filter.attribute}${index}`} alignItems="center">
